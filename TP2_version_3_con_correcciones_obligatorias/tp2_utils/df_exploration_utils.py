...
def missing_matrix(df, title):
    """
    Shows missing values matrix for a dataframe.
    ...
    Output:
    ---------
       Shows the figure. No return is given
    """
    truncated_labels = [col[:10]+'...' if len(col) > 10 else col for col in df.columns]
    matrix(df)
    plt.title(title, fontsize=30)
    plt.xticks(range(len(df.columns)), truncated_labels, rotation=45)
    plt.show()
