...
from .df_exploration_utils import missing_matrix
...
def ordinal_encoding(X_train, X_test, categorical_cols):
    """
    Makes a ordinal encoding over categorical variables in X_train.
    ...
    # Creates dataframes with new columns
    df_train_encoded = pd.DataFrame(X_train_encoded, columns=categorical_cols)
    df_test_encoded = pd.DataFrame(X_test_encoded, columns=categorical_cols)

    # Concatenates with the original dataframes
    df_train_final = pd.concat([X_train.drop(categorical_cols, axis=1), df_train_encoded], axis=1)
    df_test_final = pd.concat([X_test.drop(categorical_cols, axis=1), df_test_encoded], axis=1)
    
    # See how the encoder transformed the data
    print("Transforming data by..")
    for col_idx, categories in enumerate(encoder.categories_):
        print(f"\nColumna {col_idx}:")
        for val_idx, category in enumerate(categories):
            print(f"\t{category} -> {val_idx}")
        print("\n")
...
def simple_imput_categorical_Values(df_train, df_test, categorical_values):
    """
    Returns copies of dataframes of train and test sets with its categorical values imputed. A simple imputer by the most frequent value is used. Also this function shows the before and after the categorical values´s distribution and missing data structure.

    Parameters:
    -----------
    df_train : pandas.Dataframe
    df_test : pandas.Dataframe
    categorical_values : list of str
        Columns´s names of categorical variables to be imputed.
    """
    # Impute train set
    simp_imp = SimpleImputer(strategy='most_frequent')
    x_train_imp = simp_imp.fit_transform(df_train[categorical_values])
    # Impute test set
    x_test_imp = simp_imp.transform(df_test[categorical_values])

    # Transform in df
    df_train_2 = pd.DataFrame(x_train_imp,
                                        columns=categorical_values,
                                        index=df_train.index)
    df_test_2 = pd.DataFrame(x_test_imp,
                                        columns=categorical_values,
                                        index=df_test.index)

    # Use the generated dataframes to replaces the columns in the originals
    df_train_imputed = pd.concat([df_train.drop(categorical_values, axis=1), df_train_2], axis=1)
    df_test_imputed = pd.concat([df_test.drop(categorical_values, axis=1), df_test_2], axis=1)

    # Compare the before and after of the categorical variables´s distributions
    dataframes = [df_train, df_train_imputed]
    titles = ['Sin imputar categóricas', 'Imputando categóricas']
    ...
    # Compare the before and after of the missing values structure
    missing_matrix(df_train, "Estructura de datos faltantes en\nDATASET A\nANTES de imputaciones categóricas")
    missing_matrix(df_train_imputed, "Estructura de datos faltantes en\nDATASET A\nDESPUÉS de imputaciones categóricas")
